...
namespace DZ顶贴机X2
{
    ...
    ///<param name="e"></param>   
    public delegate void WebClientUploadEvent(object sender, DZ顶贴机X2.UploadEventArgs e);
    ...
    ///<param name="sender"></param>   
    ///<param name="e"></param>   
    public delegate void WebClientDownloadEvent(object sender, DZ顶贴机X2.DownloadEventArgs e);
    ...
    public class WebClient
    {
        ...
        private const string BOUNDARY = "--CUPID--";
        ...
        ArrayList alCookies = new ArrayList();
                ...
                if (line != "")
                {
                    ...
                    if (M.Success)
                    {
                        ...
                        if (M.Groups[1].Value == "Set-Cookie")
                        {
                            ...
                            //------- 重复cookie的处理 --------
                            string strCookie_Temp = Regex.Match(M.Value.Trim(), ".+?=").Value;
                            string strCookie_Temp2 = Regex.Match(M.Value.Trim(), "=.+").Value;
                            int j = 0;
                            for (int i = 0; i < alCookies.Count; i++)
                            {
                                string strTemp = alCookies[i].ToString();
                                if (strTemp.Contains(strCookie_Temp) || strTemp.Contains(strCookie_Temp2))
                                {
                                    alCookies.RemoveAt(j);
                                    break;
                                }
                                j++;
                            }
                            alCookies.Add(M.Value.Trim() + ";");

                            cookie = null;
                            if (alCookies.Count > 0)
                            {
                                foreach (string strTemp_Cookie in alCookies)
                                {
                                    cookie += strTemp_Cookie;
                                }
                            }
                            //---------------------------------                             
                            //cookie += M.Value.Trim() + ";";
